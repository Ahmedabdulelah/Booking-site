...
* `templates/forms` -- (Already complete.) Defines the forms used to create new  Publications، shows, and venues.
...
4. Fill out every `TODO` section throughout the codebase. We suggest going in order of the following:
    ...
    * Using SQLAlchemy, set up normalized models for the objects we support in our web app in the Models section of `app.py`. Check out the sample pages provided at /Publications/1, /venues/1, and /shows for examples of the data we want to model, using all of the learned best practices in database schema design. Implement missing model properties and relationships using database migrations via Flask-Migrate.
    * Implement form submissions for creating new Venues, Publications, and Shows. There should be proper constraints, powering the `/create` endpoints that serve the create form templates, to avoid duplicate or nonsensical form submissions. Submitting a form should create proper new records in the database.
    * Implement the controllers for listing venues, Publications, and shows. Note the structure of the mock data used. We want to keep the structure of the mock data.
...
3. The application should behave just as before with mock data, but now uses real data from a real backend server, with real search functionality. For example:
  ...
  * I should be able to go to the URL `/Publications/<Publications-id>` to visit a particular artist’s page using a unique ID per artist, and see real data about that particular artist.
  * Venues should continue to be displayed in groups by city and state.
  * Search should be allowed to be partial string matching and case-insensitive.
  * Past shows versus Upcoming shows should be distinguished in Venue and Publications pages.
  * A user should be able to click on the venue for an upcoming show in the Publications page, and on that Venue's page, see the same show in the Venue Page's upcoming shows section.
...
*  Implement Publications availability. An Publications can list available times that they can be booked. Restrict venues from being able to create shows with Publications during a show time that is outside of their availability.
* Show Recent Listed Publications and Recently Listed Venues on the homepage, returning results for Artists and Venues sorting by newly created. Limit to the 10 most recently listed items.
* Implement Search Publications by City and State, and Search Venues by City and State. Searching by "San Francisco, CA" should return all Publications or venues in San Francisco, CA.
